...
void LedTabBar::insertAssociationSubMenu(const NickInfoPtr &nickinfo)
{
    ...
    if(!addr.isEmpty())
    {
        ...
        if(addr.preferredEmail().isEmpty())
        {
            ...
            m_popup->setWhatsThis(SendEmail, i18n("Sends an email to this contact using the preferred email address set in the contact's addressbook association.  This is currently disabled because the associated contact does not have any preferred email address set."));
        }
        else
        {
        ...
        if(lookTab->rect().contains(ce->pos()))
        {
            ...
            if(win)
            {
                ...
                if(viewType == ChatWindow::Query)
                {
                    ...
                    {
                                                  //This _does_ happen when the user goes offline!
                        m_popup->setItemEnabled(AddressbookSub, false);
                        m_popup->setItemEnabled(SendEmail, false);
                ...
                if(win->isChannelEncodingSupported())
                {
                    ...
                    int encodingIndex=Konversation::IRCCharsets::self()->shortNameToIndex(encoding);
                                                  // identity default
                    m_popupEncoding->setItemChecked(POPUPID_ENCODING_OFFSET+0, (encoding.isEmpty()));
