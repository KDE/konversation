...
void InputFilter::parseClientCommand(const QString &prefix, const QString &command, QStringList &parameterList, QHash<QString, QString> messageTags)
{
    ...
    if (command == QStringLiteral("privmsg") && plHas(2))
    {
        ...
        if (!trailing.isEmpty() && trailing.at(0)==QChar(0x01))
        {
            ...
            if (hasArg)
                ctcpArgument = konv_app->doAutoreplace(ctcpArgument, false)[0].toString();
            ...
            else if (ctcpCommand == QStringLiteral("version"))
            {
                if(!isIgnore(prefix,Ignore::CTCP))
                {
                    ...
                    else
                    {
                        ...
                        reply = QString(QStringLiteral("Konversation %1 Build %2 (C) 2002-2017 by the Konversation team"))
...
void InputFilter::parsePrivMsg(const QString& prefix, QStringList& parameterList, const QHash<QString, QString> &messageTags)
{
    ...
    Application* konv_app = Application::instance();
    message = konv_app->doAutoreplace(message, false)[0].toString();
