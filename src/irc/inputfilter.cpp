...
void InputFilter::parseClientCommand(const QString &prefix, const QString &command, QStringList &parameterList)
{
    ...
    if (command == "privmsg" && plHas(2))
    {
        ...
        // CTCP message?
        if (m_server->identifyMsg() && (trailing.length() > 1 && (trailing.at(0) == '+' || trailing.at(0) == '-')))
        {
            ...
            else if (ctcpCommand == "version")
            {
                if(!isIgnore(prefix,Ignore::CTCP))
                {
                    ...
                    else
                    {
                        ...
                        reply = QString("Konversation %1 Build %2 (C) 2002-2011 by the Konversation team")
...
void InputFilter::parseServerCommand(const QString &prefix, const QString &command, QStringList &parameterList)
{
    ...
    if (!isNumeric)
    {
        if (command == "ping")
        {
            ...
            m_server->queue("PONG"+text, Server::HighPriority);
        ...
        switch (numeric)
        {
            ...
            case RPL_UMODEIS:
            {
                if (plHas(0))
                {
                    ...
                    m_server->appendMessageToFrontmost("Info", message);
                }
                break;
            }
            ...
            case RPL_NAMREPLY:
            {
                if (plHas(4))
                {
                    ...
                    m_server->addPendingNickList(parameterList.value(2), nickList); // TEST this was a 2
            ...
            case RPL_WHOREPLY:
            {
                if (plHas(6))
                {
                    ...
                    {
                        if (getAutomaticRequest("WHO",m_whoRequestList.front())==0)
                        {
            ...
            case RPL_BANLIST:
            {
                ...
                if (plHas(3))
                {
                    ...
                    {
                        m_server->addBan(parameterList.value(1), parameterList.join(" ").section(' ', 2, 4)); //<-- QString::Section handles out of bounds end parameter
                    }
                    else
                    {
                        QDateTime when;
            ...
            { // TODO make sure this works, i amputated the "+ ' '+trailing"
                if (plHas(0))
                {
                    m_server->appendStatusMessage(i18n("Users"), parameterList.join(" ").section(' ',1));
                }
                break;
            }
            ...
            case RPL_CAPAB: // Special freenode reply afaik
            {
                if (plHas(2))
                {
                    ...
                    {
                        m_server->appendMessageToFrontmost(command, parameterList.join(" ").section(' ',1) + ' '+trailing);
                    }
                }
                break;
            }
