...
{
    bool ok = false;
    QString themeName = QInputDialog::getText(this, i18n("Save Color Theme"), i18n("Theme name:"), QLineEdit::Normal, QString(), &ok);

    if(ok)
    {
        QString themePath = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QStringLiteral("/konversation/colorthemes/");
        QSettings colorSettings(themePath + themeName, QSettings::IniFormat);
...
void ColorsAppearance_Config::loadThemes()
{
    ...
    QStringList paths = QStandardPaths::locateAll(QStandardPaths::GenericDataLocation, "konversation/colorthemes/", QStandardPaths::LocateDirectory);
    ...
    foreach(const QString& path, paths)
    {
        ...
        foreach(const QString& fileName, dir.entryList(QDir::Files))
        {
            ...
            if(file.exists())
            {
                QSettings theme(file.absoluteFilePath(), QSettings::IniFormat);
                QString themeName = theme.value("KonversationTheme/Name").toString();

                if(!themeName.isEmpty())
                    themeCombo->addItem(themeName, file.absoluteFilePath());
            }
        }
    }
}
