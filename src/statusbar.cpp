...
void KonversationStatusBar::setTooLongLag(Server* lagServer, int msec)
{
    if ((msec % 5000)==0)
    {
        ...
        {
            lagString = i18np("No answer from server %1 for more than 1 second.", "No answer from server %1 for more than %2 seconds.", lagServer->getServerName(), QString::number(seconds) );
        }
